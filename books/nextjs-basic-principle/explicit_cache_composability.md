...
Next.jsは**デフォルトで高いパフォーマンスと優れた開発者体験の両立**をコンセプトとして重視しています。特に、App Routerはキャッシュファーストな設計によりデフォルトで高いパフォーマンスを実現してきましたが、開発者体験についてはコミュニティから不評で、筆者自身も複雑性が高いと感じていました。
...
**Dynamic IO**は、Dynamic I/O処理を扱う際に`<Suspense>`を必須とする**制約機能**です。従来Next.jsではDynamic I/O処理を`<Suspense>`なしで扱うことができたため、Dynamic I/O処理によってレンダリングがブロックされ、TTFBやFCPが低速になる可能性がありました。これを回避するため従来は積極的なキャッシュ活用がされていましたが、Dynamic IOでは制約により`<Suspense>`の外側^[Dynamic IOはPPRと組み合わせることが前提となっているため、`<Suspense>`境界の外側はStatic Shellとなります。]にはDynamic I/O処理が含まれなくなるため、Static Shellのレスポンスをブロックせず、TTFBやFCPを高速に保つことができます。
