...
**Dynamic IO**は、動的I/O処理を扱う際に`<Suspense>`を必須とする**制約機能**です。従来Next.jsでは動的I/O処理を`<Suspense>`なしで扱うことができたため、動的I/O処理によってレンダリングがブロックされTTFBやFCPが低速になる可能性がありました。Dynamic IOでは制約により`<Suspense>`の外側には動的I/O処理が含まれなくなるため、レンダリングがブロックされず、TTFBやFCPを高速に保つことができます。
