...
**Dynamic IO**は、動的I/O処理を扱う際に`<Suspense>`を必須とする**制約機能**です。従来Next.jsでは動的I/O処理を`<Suspense>`なしで扱うことができたため、動的I/O処理によってレンダリングがブロックされ、TTFBやFCPが低速になる可能性がありました。これを回避するため従来は積極的なCache活用がされていましたが、Dynamic IOでは制約により`<Suspense>`の外側には動的I/O処理が含まれなくなるため、レンダリングがブロックされず、TTFBやFCPを高速に保つことができます。
...
従来は開発者がDynamic Renderingを意識するのに暗黙的なAPIと広範な実装把握が必要でしたが、Dynamic IOでは`<Suspense>`よりDynamic Renderingの境界が明確になりました。
