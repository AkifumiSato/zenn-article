...
Dynamic Contentはネットワーク通信などを伴うDynamicな処理や、[Runtime Data↗︎](https://nextjs.org/docs/app/getting-started/cache-components#runtime-data)、[非決定的操作↗︎](https://nextjs.org/docs/app/getting-started/cache-components#non-deterministic-operations)を扱うような`<Suspense>`境界の内側^[[事前レンダリング時に解決されうるような動的処理↗︎](https://nextjs.org/docs/app/getting-started/cache-components#automatically-prerendered-content)はStatic Shell内でも扱うことが可能です]のコンテンツを指します。
...
Routeに`loading.tsx`が存在する場合には暗黙的に`<Suspense>`境界が設定されるため、ページ側で明示的な`<Suspense>`なしにDynamic I/O処理を扱うことができます。
...
### ページやレイアウトの一部のみがDynamicなRoute

ページやレイアウトの一部のみがDynamicな場合には、明示的に`<Suspense>`境界を追加しましょう。
...
このように、大部分は事前レンダリング可能で一部のみがDynamicなRouteにおいては、ページやレイアウト内で明示的に`<Suspense>`境界を追加することを推奨します。
