...
Cache Componentsは、PPRとCacheの改善により、**デフォルトで高いパフォーマンス**と**優れた開発者体験**の両立を目指したものです。

開発者はデータフェッチなどの動的な処理を扱う際に、`"use cache"`による静的化か`<Suspense>`によるリクエスト時までレンダリングを遅延するか、どちらかを選択する必要があります。
