...
- Static Shellを拡張したり、リクエスト間でCacheを共有したりできる。ただし...
  - https://github.com/vercel/next.js/issues/85240#issuecomment-3560124078
    - `"use cache"`はCache可能なことをNext.jsに伝える物であり、必ずしもCacheされるとは限らない、プラットフォームや設定による
    - マルチプロセスのセルフホスティングにおいてはCache不整合の原因になり得るし、VercelのCacheHandlerは`"use cache"`によるインメモリCacheを無効化してるので、リクエスト間でCacheを共有するなら`"use cache: remote"`を使いましょう（次章にて解説）
