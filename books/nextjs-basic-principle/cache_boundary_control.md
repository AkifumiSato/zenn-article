...
`"use cache"`はキャッシュの境界を宣言するもので、キャッシュの制御は制御関数によって行います。従来の設定による制御と比べると、制御関数は高い自由度とより詳細な設定が可能になっています。

## 背景

[明示的で合成可能なキャッシュ戦略](explicit_cache_composability)で解説したように、Cache Componentsでは`"use cache"`を関数やファイルで宣言することで、関数やコンポーネントが**キャッシュ可能である**ことを表明します。
...
これは、従来の暗黙的で多層のキャッシュ戦略とは全く異なるものです。

### 従来のキャッシュ境界と制御

従来のキャッシュは多層で、特にサーバーサイドにおいてはFull Route CacheとData Cacheの役割と相互影響などを考慮する必要がありました。
...
Cache Componentsでは`"use cache"`で関数やコンポーネントをキャッシュするため、従来よりも影響範囲が明確で、RSCとの親和性が高い設計となっています。
...
`"use cache"`は、Static Shellに動的な処理やコンポーネントを含めることを主なユースケースとして想定していますが、Dynamic Content内から参照することも可能で、この場合にはインメモリキャッシュ^[次章の[`cacheHandlers.default`](remote_cache#use-cache-default)を設定することで、保存先を変更することができます。]のマーカーとして機能します。

しかし、現代のホスティング環境は多くの場合複数プロセスで動作するため、インメモリなキャッシュはリクエスト間で共有できません。リクエスト間でキャッシュを共有したい場合には、`"use cache"`ではなく、次章で解説する[`"use cache: remote"`](remote_cache)を利用しましょう。

### `cacheLife(profile)`

[`cacheLife(profile)`↗︎](https://nextjs.org/docs/app/api-reference/functions/cacheLife)は、キャッシュの有効期間を設定するための関数です。`profile`はキャッシュの有効期間に関する以下3つのタイミングを指定することができ、これらをまとめた設定であるprofile文字列やObjectを渡すことができます。
...
[`cacheTag(...tags)`↗︎](https://nextjs.org/docs/app/api-reference/functions/cacheTag)は、キャッシュにtaggingするためのAPIです。`updateTag()`や`revalidateTag()`で指定したtagが付与されたキャッシュを更新することができます。
...
[`updateTag(tag)`↗︎](https://nextjs.org/docs/app/api-reference/functions/updateTag)は、対象のtagに関連するキャッシュを**即座に更新**するためのAPIです。更新された結果は、即座に画面に反映されます。

一方、[`revalidateTag(tag)`↗︎](https://nextjs.org/docs/app/api-reference/functions/revalidateTag)も同様にキャッシュを更新するためのAPIですが、こちらはバックグラウンドで更新を行い、その間は古いキャッシュを参照します。
