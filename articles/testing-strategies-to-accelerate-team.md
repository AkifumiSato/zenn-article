...
本稿では「単体テストの考え方/使い方」に沿ってjestで行ってる単一プロセスで実行可能なテストのことを単体テストと呼称していますが、どの考え方をもとに呼称を決めるかはプロジェクトやチームによってそれぞれで良いと考えます。呼称や定義の統一は重要ですが、世間的にさまざまな解釈がある以上プロジェクト内において統一できてれば十分です。
:::
...
フロントエンドの場合、[フロントエンド開発のためのテスト入門](https://www.shoeisha.co.jp/book/detail/9784798178189)でテスティングフレームワークの使い方から後述のテスト設計・戦略まで網羅的・効率的に学べるので、お勧めです。
...
テスティングピラミッドの中間層はフロントエンドにおいては、Unitはstorybook、IntegrationはAPI通信を伴うコンポーネントや処理のjestと近い位置付けであると筆者は考えています。storybookをテストというかは諸説ありそうですが、テスティングピラミッドのStaticは静的解析の話なので筆者はあまり違和感がありません。
...
上記のように段階的にチームに導入すれば、開発チームは効果的にテストが運用できるようになるのでしょうか？

筆者が実際に2つのチームでやってみた限りでは、上記の段階を経て今日では効果的で効率的なテスト運用ができていると思える状態まで至りました。もちろんこれは筆者の主観的な感想なので、テスト含め技術的な振り返りを匿名アンケートという形でチームメンバーにお願いしました。
