...
リグレッション検知に役立たないテストは書くべきではないですし、書き方やテスティングフレームワークの使い方に悩むのならば有識者やチームメンバーにまずは相談すべきです。あまりに時間がかかるようなら、時には諦めることも重要です。
...
正しくテストを運用するには、相応の知識とスキルが必要になってきます。具体的にはテスティングフレームワークやテスト設計自体のハードスキルと、それらを知識やスキルをチーム内でお互いに共有しチーム全体で統一された運用をするソフトスキルです。ハードスキルは少数のリードエンジニアの手腕によって得られる費用対効果が変わってきます。ソフトスキルについてはチームの心理的安全性や文化が大きく影響してきます。

### テスティングフレームワークの知識やテスト設計

これらはテストに関してリードするエンジニアが率先してハードスキルを習得している必要があります。もしテストを導入したいと思っているならば、まずは率先してハードスキルを習得しましょう。

Jestについては公式ドキュメントや[フロントエンド開発のためのテスト入門](https://www.shoeisha.co.jp/book/detail/9784798178189)を読んで学ぶことをお勧めします。筆者の場合、フロントエンド開発環境へのJest導入は何度も行っていたので最低限理解はあるつもりでしたが、テスト設計については体系的に学んだことがなかったため、前述の[単体テストの考え方/使い方](https://book.mynavi.jp/ec/products/detail/id=134252)や[フロントエンド開発のためのテスト入門](https://www.shoeisha.co.jp/book/detail/9784798178189)、[テスト駆動開発](https://shop.ohmsha.co.jp/shopdetail/000000004967/)や[Clean Craftsmanship](https://tatsu-zine.com/books/clean-craftsmanship)などを読んでテスト設計について理解を深めました。これらはどれも非常におすすめですが、Clean Architectureで知られるアンクル・ボブ著のClean Craftsmanshipはテストに限らずテストや設計・スタンスなど幅広く言及しており、とてもおすすめです。

### 知識やスキルの共有の難しさ

ハードスキルは学ぶべき方向性が公式ドキュメントや書籍などから得られる一方、チーム開発で知識やスキルを共有するソフトスキルはチームマネジメントスキルに繋がるので一朝一夕で書籍などから学べるものでもありません。エンジニアの場合多くはドキュメントを書いて知識を共有しようとしますが、用意したドキュメントに以下のような経験もしくはフィードバックを受けた方は多いのではないでしょうか？少なくとも筆者はどれも大いに心当たりがあります。
...
これらを全て越えて、ようやく知識とスキルの共有が達成されます。知識とスキルの共有は、本当に難しいことです。ではどうすればチーム開発でテストを正しく運用できるのでしょう？

以降はこの問いに対する、戦略の話になります。

## テスト戦略

テストにはさまざまな粒度が存在し、しかもその定義はさまざまな解釈が存在します。単体テストにおける「古典学派」「ロンドン学派」などはまさに時代や人によって異なる点でしょう。

https://zenn.dev/akfm/articles/frontend-unit-testing#%E3%80%8C%E5%8D%98%E4%BD%93%E3%80%8D%E3%81%AE%E5%AE%9A%E7%BE%A9

一方で共通認識とされているのが、より上位のテストであるE2Eなどは再現性は高いがコストも高く、速度・安定性に欠けるという点です。この点から考えられた粒度ごとの最適なバランスの図として有名なのが[テストピラミッド](https://gihyo.jp/dev/serial/01/savanna-letter/0005)や[テスティングトロフィー](https://kentcdodds.com/blog/the-testing-trophy-and-testing-classifications)です。
...
前述の通り、テストの最大の目的は開発速度の維持で、これを限られている時間の中で最適化することです。筆者の場合担当していたのがフロントエンド開発だったので、後者のテストピラミッドの方が適してると判断し、低レイヤーなコンポーネントのテストは薄く、データ接続を伴うコンポーネントや処理に対しては厚くテストしていくこととしました。

### テスト粒度の定義

前述の[テストピラミッド](https://gihyo.jp/dev/serial/01/savanna-letter/0005)の記事には、最も有名なプログラマーの1人であるマーティン ・ファウラーの引用が載っています。
...
それほどに「単体テスト」の指す言葉の定義は人によりけりで、異なります。少しフロントエンドの話になりますが、APIなどの外部システムとの「統合」があればインテグレーションテストと呼ぶ人もいるし、上記の記事のように複数のモジュールからなるもののテストをインテグレーションテストと呼ぶ人もいます。単体テストの考え方/使い方では、「単一プロセスで実行可能なテスト」のことを単体テストと定義しています。筆者はこれらの世間でも別れてる解釈についてどれを採用するか考えること以上に、storybookとjestそれぞれで何をやって何をやらないかを定義することの方が重要だと考えました。一応「単体テストの考え方/使い方」に沿ってjestで行ってるテストのことを単体テストと呼称することにしましたが、どの考え方をもとに呼称を決めるかはプロジェクトやチームによってそれぞれで良いと考えます。
...
単体テストの定義は前述の通りで、筆者は単体テスト（storybook+jest）に注力することが費用対効果が高いと判断しました。
